...
export class ScoresComponent implements OnInit {
  ...
  constructor(private boxScoreService: BoxScoreService, private colorService: ColorService, private dateService: DateService) {}
  ...
  public getLeagueCode(boxScore: MiniBoxScore): string {
    return (boxScore.isArcade ? boxScore?.developmentPhase?.name : boxScore?.league?.code).toUpperCase();
  }
  ...
  public getShotsOnGoal(boxScore: MiniBoxScore, team: 'away' | 'home'): number {
    return boxScore?.periodSummaries?.reduce((acc, periodSummary) => {
  ...
  public setTeamNameTooltip(boxScore: BoxScore, isAway: boolean): string {
    if (isAway) {
      ...
      if (!awayTooltip.replace(' ', '')) {
        awayTooltip = 'Away Team';
      }
      // return the tooltip
      return awayTooltip;
    } else {
      ...
      if (!homeTooltip.replace(' ', '')) {
        homeTooltip = 'Home Team';
      }
      // return the tooltip
      return homeTooltip;
    }
